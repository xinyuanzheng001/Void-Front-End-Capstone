...
import SquareThumbnailImage from '../imageGallery/SquareThumbnailImage';
...
test('should not display the thumbnail image if no image list available', async () => {
  renderWithProviders(<SquareThumbnailImage displayList={[]} />);
  const result = await screen.queryAllByTestId('row-thumbnail-image');
  expect(result).toHaveLength(0);
});

test('should render the image display list when pass in', async () => {
  renderWithProviders(
  ...
  const result = await screen.queryAllByTestId('row-thumbnail-image');
  expect(result).toHaveLength(displayList.length);
});
